...
- 新規チャット開始は右ペインを未保存の新規チャット状態へ切り替え、最初の送信時にチャット生成を行う
- 既存チャット表示は個別チャット取得を利用する
- トップ画面はデスクトップ時に左サイドバーと右チャット画面の 2 ペイン構成で表示する
- モバイル時はチャット一覧をドロワー型サイドバーで開閉し、右チャット画面を主表示とする
- メッセージ送信後は定期再取得で AI 応答状態を反映する
- `pending` がなくなるか、右ペインが別チャットまたは未保存新規状態へ切り替わった時点で定期再取得を停止する
