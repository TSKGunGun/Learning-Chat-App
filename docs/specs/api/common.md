...
| `POST` | `/api/chats/{chatId}/messages/{messageId}/feedback` | 指定 AI メッセージへの Good / Bad フィードバック送信 | クライアントサイド |
...
- 既存チャットを開く操作から利用される
- 返却内容には対象チャットの `room_name` とチャット履歴を含む前提とする
...
- 対象チャットが最初のユーザーメッセージ送信前であり、ルーム名が `新規チャット` の場合は、この処理の中で最初の会話内容からチャットルーム名を自動生成して更新する
...
- 指定された AI メッセージに対する `good` または `bad` のフィードバックを受け取る
...
- 既存チャットを開く操作は `GET /api/chats/{chatId}` を利用する
...
- フィードバック送信は `POST /api/chats/{chatId}/messages/{messageId}/feedback` を利用する
