...
4. 新規チャットでは、最初のユーザーメッセージ送信時に `POST /api/chats` を通じて `chat_channels` と最初のメッセージ履歴を生成する。既存チャットでは `POST /api/chats/{channel_id}/messages` を利用する。
5. ユーザーの送信メッセージは送信直後に UI へ反映され、AI の応答生成が始まると UI は一時的な「応答中」ダミーメッセージを表示する。
6. API は回答生成処理を起動し、基盤側では過去の訂正ルールと Good / Bad フィードバックを参照して回答を生成する。AI 応答が完了すると履歴へ保存され、UI は履歴再取得により表示を更新する。
