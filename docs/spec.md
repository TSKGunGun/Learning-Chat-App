...
チャットは `chats` をチャットチャンネル、`messages` をメッセージ履歴として扱う構成を前提とする。
...
4. ユーザーがモーダル内でメッセージを送信すると、`POST /api/chats/{chatId}/messages` を通じて対象チャットにメッセージ履歴が追加され、API が回答生成処理を起動する。
5. 基盤側では、過去の訂正ルールを参照し、ルールを踏まえた回答を生成する。
6. ユーザーが特定の AI メッセージに対して訂正を送信した場合、`POST /api/chats/{chatId}/messages/{messageId}/correct` を通じて次回以降に守るべきルールを抽出し、ベクトル化して保存する。
