...
チャットは `chats` をチャットチャンネル、`messages` をメッセージ履歴として扱う構成を前提とする。チャットルーム名は新規作成時に `新規チャット` とし、最初のユーザーメッセージ送信時に会話内容から自動生成する。
...
4. ユーザーがモーダル内でメッセージを送信すると、`POST /api/chats/{chatId}/messages` を通じて対象チャットにメッセージ履歴が追加され、必要に応じてチャットルーム名が自動生成される。
5. API は回答生成処理を起動し、基盤側では過去の訂正ルールと Good / Bad フィードバックを参照して回答を生成する。
6. ユーザーは AI メッセージに対して `POST /api/chats/{chatId}/messages/{messageId}/feedback` を通じて Good / Bad フィードバックを送信でき、その評価は以後の回答生成に利用される。
7. ユーザーが特定の AI メッセージに対して訂正を送信した場合、`POST /api/chats/{chatId}/messages/{messageId}/correct` を通じて次回以降に守るべきルールを抽出し、ベクトル化して保存する。
